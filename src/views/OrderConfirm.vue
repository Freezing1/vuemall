<template>
  <div>
    ...
    <div class="container">
      <div class="checkout-order">
        <div class="page-title-normal">
          <h2 class="page-title-h2"><span>查看详情</span></h2>
        </div>
        <!-- process step -->
    ...
    <nav-footer/>
    <Modal v-bind:mdShow="errmodal" @close="closeModal">
      <p slot="message">{{errmsg}}</p>
      <div slot="btngroup">
  ...
  export default {
    ...
    methods: {
      ...
      getOrder() {
        axios.get('/goods/cartlist').then(res => {
          if (res.status === 200 && res.data.code === 0) {
            let money = 0;
            this.orderList = res.data.result.cartList.filter(v => {
              if (v.checked) {
                money += parseInt(v.productNum) * parseFloat(v.salePrice)
              }
      ...
      payment() {
        ...
        if (!addressId) {
          this.errmsg = "收货地址不能为空,请重新选择收货地址";
          this.errmodal = true;
          return false;
        }
        if (this.totalPrice === 0) {
          this.errmsg = "页面可能已经失效或者数据已经提交";
          this.errmodal = true;
          return false;
        }
        axios.post('/users/payment', {
          ...
          if (res.status === 200 && res.data.code === 0) {
            this.$router.push({path: 'paysuccess', query: {orderid: res.data.result.orderid}});
          } else {
            this.errmsg = "支付失败";
            this.errmodal = true;
          }
