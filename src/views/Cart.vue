<template>
  <div>
    ...
    <div class="container">
      <div class="cart">
        <div class="page-title-normal" v-show="!emptycart">
          <h2 class="page-title-h2"><span>我的购物车</span></h2>
        </div>
        <div v-show="!emptycart" class="item-list-wrap">
          <div class="cart-item">
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn"
                       ...
                       @click="cartnum(item.checked,item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="cartnum('minu',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="cartnum('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="showmodel(item.productName,item.productId)">
        ...
        <div v-show="!emptycart" class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleCheckAll">
                  <span class="checkbox-btn item-check-btn" v-bind:class="{'check':checkAll===true}">
        ...
        <!--当购物车是空的时候显示-->
        <div class="cart-empty" v-show="emptycart">
          <div>
            <div>
              <h1>你的购物车空空如也!</h1>
              <div class="cart-empty-div">
                <router-link to="/" class="btn btn--m">去商城逛一逛吧!</router-link>
              </div>
            </div>
          </div>
        </div>
    ...
    <nav-footer/>
    <Modal v-bind:mdShow="delcartmodal" @close="closeModal">
      <p slot="message">你确定要删除 <span style="color: red">{{delname}}</span> 吗?</p>
      <div slot="btngroup">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      delcart() {
        axios.get('/goods/delcart', {params: {productId: this.delcartid}}).then(res => {
          if (res.status === 200 && res.data.code === 0) {
            this.delname = '';
            this.delcartid = '';
            this.delcartmodal = false;
      ...
      toggleCheckAll() {
        let flage = !this.checkAll;
        this.cartList.map((item) => {
          item.checked = flage
        });
        axios.post('/goods/editcheckAll', {checked: flage}).then(res => {
          if (res.status === 200) {
            console.log(res.data);
          }
        })
    ...
    computed: {
      ...
      totalPrice() {
        ...
        this.cartList.map(v => {
          if (v.checked === true) {
            money += parseFloat(v.salePrice) * parseInt(v.productNum)
          }
        })
        return money
      }
